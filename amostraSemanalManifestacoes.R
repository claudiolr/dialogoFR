...
amostra <- function() {
        ...
        dlgMessage("Certifique-se que o filtro327 esta carregado e tratado", type = "ok")
        ...
        # Define o período da amostra
        periodo <- seq.Date(from = dmy(dlgInput("Defina a data de inicio do MES de analise no formato dd-mm-yyyy (sem aspas)")$res),
                            to = dmy(dlgInput("Defina a data de fim do MES de analise no formato dd-mm-yyyy (sem aspas)")$res), by = "day")
        
        
        
        amostra1 <- as.numeric(dlg_input("Novos registros: defina o tamanho da amostra.\nSe nao houver casos suficientes, a base sera gerada com valor igual a 1.")$res)
        if(has_error(monitoramento %>%
                     filter(operadorinseriu %in% atendentes$id,
                            unidadeinseriu %in% cias,
                            datareg %in% periodo) %>% 
                     group_by(operadorinseriu) %>% 
                     sample_n(size = amostra1, replace = FALSE), silent = TRUE))
        {
                NovosRegistros <- 
                        monitoramento %>% 
                        filter(operadorinseriu %in% atendentes$id,
                ...
                NovosRegistros <- 
                        ...
                        filter(operadorinseriu %in% atendentes$id,
                               ...
                               datareg %in% periodo) %>% 
                        group_by(operadorinseriu) %>% 
                        sample_n(size = amostra1, replace = FALSE)
        }
                  
        
        
        amostra2 <- as.numeric(dlg_input("Conclusoes: defina o tamanho da amostra.\nSe nao houver casos suficientes, a base sera gerada com valor igual a 1.")$res)
        if(has_error(monitoramento %>% 
                     filter(responsavelconclusao %in% atendentes$Nome,
                            dataconclusao %in% periodo) %>% 
                     group_by(responsavelconclusao) %>% 
                     sample_n(size = amostra2, replace = FALSE), silent = TRUE))
        {
                Conclusoes <- monitoramento %>% 
                        filter(responsavelconclusao %in% atendentes$Nome,
                               dataconclusao %in% periodo) %>% 
                        group_by(responsavelconclusao) %>% 
                        sample_n(size = 1, replace = FALSE)
                ...
                Conclusoes <- monitoramento %>% 
                        filter(responsavelconclusao %in% atendentes$Nome,
                               dataconclusao %in% periodo) %>% 
                        group_by(responsavelconclusao) %>% 
                        sample_n(size = amostra2, replace = FALSE)
        }        
        
        
        amostra3 <- as.numeric(dlg_input("Encaminhamentos: defina o tamanho da amostra.\nSe nao houver casos suficientes, a base sera gerada com valor igual a 1.")$res)
        if(has_error(monitoramento %>% 
                     filter(ultimoEncaDE %in% atendentes$Nome,
                            ultimoencaData %in% periodo) %>%
                     group_by(ultimoEncaDE) %>% 
                     sample_n(size = as.numeric(amostra3), replace = FALSE))) {
                Encaminhamentos <- 
                        monitoramento %>% 
                        filter(ultimoEncaDE %in% atendentes$Nome,
                               ultimoencaData %in% periodo) %>%
                ...
                Encaminhamentos <- 
                        ...
                        filter(ultimoEncaDE %in% atendentes$Nome,
                               ultimoencaData %in% periodo) %>%
                        group_by(ultimoEncaDE) %>% 
                        sample_n(size = amostra3, replace = FALSE)
        }
        
        
        
        amostra4 <- as.numeric(dlg_input("Demandas: defina o tamanho da amostra.\nSe nao houver casos suficientes, a base sera gerada com valor igual a 1.")$res)
        if(has_error(monitoramento %>%
                     ...
                     sample_n(size = as.numeric(amostra4), replace = FALSE))) {
                Demandas <- 
                        monitoramento %>%
                        separate(ultimoEncaDEEnc, into = c("ultimoEncaDEEncId", "ultimoEncaDEEncNome"), sep = " - ") %>% 
                        mutate(ultimoEncaDEEncId = str_squish(ultimoEncaDEEncId)) %>% 
                ...
                Demandas <- 
                        ...
                        filter(ultimoEncaDEEncId %in% atendentes$id,
                               ymd(ultimoEncaDataEnc) %in% periodo) %>% 
                        group_by(ultimoEncaDEEncId) %>% 
                        sample_n(size = amostra4, replace = FALSE)
        }
        ...
        write.xlsx(list(Infos = c(paste0("Dados referentes ao período de ",
                        ...
                        Demandas = Demandas), paste0("MonitoramentoPreenchimento",format(Sys.time(), "%Y%m%d_%Hh%Mm"),".xlsx"),
