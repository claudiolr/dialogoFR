...
analiseCenario <- function() {
        ...
        periodoGeral <- seq.Date(from = dmy("05-11-2015"), to = mesAtualDiaFim, by = "day")
        trimestre <- seq.Date(from = trimestreDiaInicio, to = mesAtualDiaFim, by = "day")
        mesAtual <- seq.Date(from = mesAtualDiaInicio, to = mesAtualDiaFim, by = "day")
        ...
        ## 4. Quantidade e % de manifestações por assunto (PG) no último mês
        # Geral
        M4Geral <- 
                cenario %>% 
        ...
        ## 5. Quantidade e % de manifestações por assunto/tema (PG + detalhamento) no último mês
        # Geral
        M5Geral <- cenario %>% 
        ...
        ## 1. Total de demandas individuais existentes (nov/15 até atual)
        # Geral
        D1Geral <- cenario %>% 
        ...
        ## 2. Status de todas as demandas individuais (nov/15 até atual)
        # Geral
        D2Geral <- cenario %>% 
                filter(!is.na(statusdemanda)) %>% 
        ...
        ## 3. Quantidade de demandas individuais por assunto (PG) e status (nov/15 atual)
        # Geral
        D3Geral <- cenario %>% 
                filter(!is.na(statusdemanda)) %>% 
