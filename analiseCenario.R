...
cenario$manifestacaoAssuntoTema <- str_replace_all(cenario$manifestacaoAssuntoTema, c("PG001 Levantamento e Cadastro" = "PG01",
...
cenario$ManifestacaoAssunto <- str_replace_all(cenario$ManifestacaoAssunto, c("PG001 Levantamento e Cadastro" = "PG01",
                                                                              "PG002 Ressarcimento e Indenização" = "PG02",
                                                                              "PG003 Proteção e Recuperação da Qualidade de Vida dos Povos Indígenas" = "PG03",
                                                                              "PG004 Proteção e Qualidade de Vida de Outras Comunidades Tradicionais" = "PG04",
                                                                              "PG005 Proteção Social" = "PG05",
...
cenario %>% 
     ...
     group_by(manifestacaoAssuntoTema) %>% 
     summarise(Total = n()) %>% 
     mutate(Percentual = round(prop.table(Total), digits = 3)*100) %>% 
...
cenario %>% 
     mutate(statusManifestacao = ifelse(statusManifestacao %in% c("Respondida",
                                                                  "Respondida (não se enquadra nas políticas de indenização e auxilio financeiro atuais)",
                                                                  "Respondida no ato"), "Finalizada", "Não finalizada")) %>% 
     group_by(manifestacaoAssuntoTema, territorio) %>% 
     summarise(Total = n()) %>% 
     pivot_wider(names_from = territorio, values_from = Total, values_fill = list(Total = 0))



## 5 Quantidade e % de manifestações por assunto (PG) no último mês (março/20)
# Geral
cenario %>% 
     filter(datareg >= ymd("2020-03-01") & datareg <= ymd("2020-03-31")) %>% 
     mutate(statusManifestacao = ifelse(statusManifestacao %in% c("Respondida",
...
cenario %>% 
     ...
     mutate(statusManifestacao = ifelse(statusManifestacao %in% c("Respondida",
                                                                  "Respondida (não se enquadra nas políticas de indenização e auxilio financeiro atuais)",
                                                                  "Respondida no ato"), "Finalizada", "Não finalizada")) %>% 
     group_by(territorio, ManifestacaoAssunto, statusManifestacao) %>% 
     summarise(Total = n()) %>% 
     pivot_wider(names_from = statusManifestacao, values_from = Total, values_fill = list(Total = 0))
)
...
demandaTerritorio <- split(demandaTerritorio, demandaTerritorio$territorio)



setwd("C:/Users/Claudio/HERKENHOFF & PRATES/HERKENHOFF & PRATES/Fundação Renova Diálogo - Execução/OrganizacaoInformacao")
